...
TODO(content-move): the embedded `forward` templates do
`from marlib.adapters.mas_zero.core import Info, LLMAgentBase`, a path that no
longer exists after this system was externalised (it now loads under
`_marlib_systems.mas_zero`). When reviving mas_zero, register the package under
a stable `sys.modules` alias and update this import path. See prompts.py.
"""
...
RAG_COT = {
    ...
    from marlib.adapters.mas_zero.core import Info, LLMAgentBase
