"""Meta-agent prompts for ADAS RAG architecture generation.

The meta-model uses these prompts to generate a single forward() function that
combines retrieval tools with reasoning blocks (ADAS / Meta Agent Search style,
single-step — no decomposition or refinement loop).
...
IMPORTANT: `Info` and `LLMAgentBase` are already available in scope.
Do NOT import them (no `from ... import ...` for these). You may import
stdlib modules you need (e.g. `from collections import Counter`).
