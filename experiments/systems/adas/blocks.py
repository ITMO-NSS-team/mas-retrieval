"""RAG-adapted reasoning blocks for ADAS.
...
These are adapted from the classic ADAS / Meta Agent Search building blocks
(COT, Reflexion, Debate, COT-SC) with retrieval embedded into the reasoning
flow.

`Info` and `LLMAgentBase` are injected into the exec() globals by the adapter
(see adapter.py), so the forward() bodies reference them directly without an
import. Stdlib imports (e.g. `from collections import Counter`) are fine.
"""
