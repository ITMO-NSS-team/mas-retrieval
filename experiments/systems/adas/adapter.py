...
Single-step meta-agent code generation (ADAS / Meta Agent Search style): an LLM
designs ONE agent architecture from building blocks with RAG tools, which is
then executed per question. This deliberately omits MAS-Zero's defining steps
(per-question meta-iteration with MAS-Feedback, task decomposition into sub-MAS,
and self-verification); see the `mas_zero` system for the full algorithm.
...
from .tracing import ADASTrace, AgentTrace
...
@register("adas")
class ADASAdapter(AbstractAdapter):
    """ADAS-style single-step meta-agent adapter for RAG benchmarks.

    The meta-model generates ONE architecture (forward function) that combines
    reasoning blocks with retrieval tools. The architecture is then executed
    per-question with access to retrieve(), rerank(), and calculate(). Unlike
    MAS-Zero, there is no evaluate/refine loop and no self-verification.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        logger.info(
            "ADASAdapter: mode=%s, meta_model=%s, model=%s, blocks=%d, trace=%s",
    ...
    @property
    def name(self) -> str:
        return f"adas_{self._generation_mode}"
    ...
    def generate_system(self, question: str) -> str:
        ...
        if solution is not None:
            ...
            logger.info("ADAS generated architecture:\n%s", desc)
            return desc
    ...
    def execute(
        ...
        try:
            ...
            logger.error("ADAS execution failed: %s", e)
    ...
    def _save_trace(self, trace: ADASTrace) -> None:
        """Save trace to logs/adas/ as JSON and log summary."""
        logger.debug("ADAS trace:\n%s", trace.summary())
        try:
            trace_dir = os.path.join("logs", "adas")
