"""System prompts for MA-RAG agents, ported from MA-RAG/src/prompt_template.py."""

PLAN_SYSTEM_PROMPT = """\
You are tasked with assisting users in generating structured plans for answering questions. Your goal is to deconstruct a query into manageable, simpler components. For each question, perform the following tasks:
...
Include all of information from previous step's results in the query if it is relevant, especially for aggregate tasks.
Be concise.\
"""

EXTRACT_SYSTEM_PROMPT = """\
Summarize and extract all relevant information from the provided passages based on the given question. Remove all irrelevant information. Think step-by-step.
...
You are an assistant for question-answering tasks. Use the following process to deliver concise and precise answers based on the retrieved context. If all of retrieved context are not relevant, answer based on general knowledge.
...
ANSWER_AGGREGATE_SYSTEM_PROMPT = """\
Answer the question from human.
...
Your task is writing a summary about a plan to solve a question and answer the question based on outputs from each step in the plan. Think step-by-step.
