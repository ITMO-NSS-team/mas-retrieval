"""Benchmark registry: directory-discovered specs + auto-registered builders.

A benchmark is defined in one place — a builder module under this package
(decorated with ``@register("name")``) plus a ``manifest.toml`` inside its data
directory::

    experiments/data/benchmarks/<name>/
        manifest.toml     # committed: description (MAS prompt), split, collection
        questions.jsonl   # generated by builder.download()
        corpus.jsonl      # generated by builder.build_corpus()
        index/            # generated by build_index() (chroma persistent dir)
        source/           # raw inputs (e.g. financebench PDFs)

Discovery scans the benchmarks root for ``manifest.toml`` files, so the set of
available benchmarks is exactly the set of present directories — there is no
central dict to edit or comment out. Build logic lives in ``BenchmarkBuilder``
subclasses keyed by the same name via ``@register``.
"""
...
DEFAULT_ROOT = Path("experiments/data/benchmarks")
...
def discover(root: str | Path = DEFAULT_ROOT) -> dict[str, BenchmarkSpec]:
    """Discover available benchmarks by scanning ``<root>/*/manifest.toml``.

    The available set is exactly the directories that carry a manifest — this
    replaces a hand-maintained registry dict. Returns an empty mapping if the
